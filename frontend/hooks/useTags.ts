...
export function useTagsForItem(itemType: "movie" | "show", itemId: number | undefined) {
  return useQuery<Tag[]>({
    queryKey: ["tags", "for-item", itemType, itemId],
    queryFn: async () => {
      if (!itemId || itemId <= 0) return [];
      const data = await api.get<Tag[]>(`/tags/for-item?item_type=${itemType}&item_id=${itemId}`);
      return data ?? [];
    },
...
export function useRemoveTagFromItem() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      await api.delete(`/tags/${tagId}/items/${itemType}/${itemId}`);
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["tags"] });
      queryClient.invalidateQueries({
