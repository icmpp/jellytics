...
export function useCollectionsForItem(itemType: "movie" | "show", itemId: number | undefined) {
  return useQuery<CollectionListItem[]>({
    queryKey: ["collections", "for-item", itemType, itemId],
    queryFn: async () => {
      if (!itemId || itemId <= 0) return [];
      const data = await api.get<CollectionListItem[]>(buildListUrl(itemType, itemId));
      return data ?? [];
    },
...
export function useAddToCollection() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["collections"] });
      queryClient.invalidateQueries({
        queryKey: ["collections", "for-item", variables.itemType, variables.itemId],
      });
      queryClient.invalidateQueries({
...
export function useRemoveFromCollection() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      await api.delete(`/collections/${collectionId}/items/${itemType}/${itemId}`);
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["collections"] });
      queryClient.invalidateQueries({
