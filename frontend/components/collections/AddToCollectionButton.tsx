...
export function AddToCollectionButton({
  ...
  const { data: collections = [], isLoading } = useCollectionsForItem(itemType, itemId);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant={variant} size={size} className={className} aria-label="Add to collection">
          <FolderPlus className={iconOnly ? "size-6" : "h-3 w-3 mr-2"} />
          {!iconOnly && "Add to collection"}
        </Button>
      </PopoverTrigger>
        ...
        {isLoading ? (
          ...
          collections.map((c) => (
            ...
            >
              ...
              {c.itemCount > 0 && (
                <span className="text-xs text-white/40 ml-auto">{c.itemCount}</span>
              )}
            </button>
          ))
        )}
