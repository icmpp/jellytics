...
import { useCollection, useRemoveFromCollection, useUpdateCollection } from "@/hooks/useCollections";
...
function CollectionItemCard({
	...
	return (
		<Link href={href} className="min-w-0 h-full flex flex-col group">
			<div className={cn(MEDIA_POSTER_CONTAINER, "hover:border-white/[0.12] transition-all")}>
				<PosterImage
					src={posterSrc}
					alt={item.title || ""}
				...
				>
					<ConfirmPopover
						...
						description={`Remove "${item.title || "this item"}" from the collection?`}
						confirmLabel="Remove"
						confirmIcon={Trash2}
						variant="destructive"
